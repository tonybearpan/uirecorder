...
var defaultOpenChecker = false;
var defaultBrowserSize = 'maximize';
...
function startRecorder(options){
    ...
    inquirer.prompt(questions).then(function(answers){
        ...
        function onReady(serverPort){
            var wdPacUrl = wdproxy ? 'http://127.0.0.1:'+serverPort+'/proxy.pac' : '';
            // recorder browser
            setTimeout(function(){
                newChromeBrowser({wdPort: wdPort, hosts: hosts, wdPacUrl: wdPacUrl, isRecorder: true, debug: debug }, function*(browser){
                    if(!mobile){
                        if (browserSize) {
            ...
            if(mobile){
                ...
                // checker browser
                setTimeout(function(){
                    newChromeBrowser({ wdPort: wdPort, hosts: hosts, wdPacUrl: wdPacUrl, isRecorder: false, debug: debug }, function*(browser){
                        if(browserSize){
                            yield browser.windowSize(browserSize[0], browserSize[1]);
                        }
                        else {
                ...
                if(continueRecord){
                    ...
                    templateContent = templateContent.replace(/\{\$(\w+)\}/g, function(all, name){
                        switch(name){
                            case 'testCodes':
                                var testCaseCodes = [];
                                testCaseCodes.push('it(\''+fileName+'\', async function(){');
                                testCaseCodes = testCaseCodes.concat(arrTestCodes);
                                testCaseCodes.push("});");
                                return testCaseCodes.join('\r\n');
                            case 'sizeCode':
                                return sizeCode;
                            case 'appPath':
...
function newChromeBrowser(options, callback) {
    ...
    if(options.wdPacUrl){
        capabilities.proxy = {
            'proxyType': 'pac',
            'proxyAutoconfigUrl': options.wdPacUrl
        };
